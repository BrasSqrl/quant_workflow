...
[v1.0.0 Release Notes](./RELEASE_NOTES_V1.0.0.md) and the
[Release Validation Report](./RELEASE_VALIDATION_REPORT.md) for validated
...
Start with [docs/README.md](./README.md). It organizes the documentation
...
- [User Quick Start Guide](./user_guides/QUICK_START.md)
- [PD Logistic Regression Walkthrough](./user_guides/PD_LOGISTIC_REGRESSION_WALKTHROUGH.html)
- [Data Requirements Guide](./user_guides/DATA_REQUIREMENTS.md)
...
- [Large Data Playbook](./user_guides/LARGE_DATA_PLAYBOOK.md)
- [Large Data Certification Guide](./user_guides/LARGE_DATA_CERTIFICATION_GUIDE.md)
- [Glossary](./user_guides/GLOSSARY.md)
...
- [docs/ENGINEERING_RUBRIC.md](./ENGINEERING_RUBRIC.md)
- [docs/UI_UX_STANDARD.md](./UI_UX_STANDARD.md)

## Transparency and Auditability Guides

The repository now includes dedicated audit-oriented reference guides:

- [docs/STATISTICAL_TEST_CATALOG.md](./STATISTICAL_TEST_CATALOG.md)
- [docs/MODEL_CATALOG.md](./MODEL_CATALOG.md)
- [docs/METRIC_CATALOG.md](./METRIC_CATALOG.md)
- [docs/PREPROCESSING_AND_DATA_TREATMENT_GUIDE.md](./PREPROCESSING_AND_DATA_TREATMENT_GUIDE.md)
- [docs/CHECKPOINT_STAGE_GUIDE.md](./CHECKPOINT_STAGE_GUIDE.md)
- [docs/GUI_TO_CODE_TRACEABILITY_GUIDE.md](./GUI_TO_CODE_TRACEABILITY_GUIDE.md)
- [docs/SAGEMAKER_SETUP.md](./SAGEMAKER_SETUP.md)
...
[docs/MACOS_SETUP.md](./MACOS_SETUP.md), and a plain-text copy is kept at
...
The detailed guide is at [docs/SAGEMAKER_SETUP.md](./SAGEMAKER_SETUP.md), and a
...
- Step 5 `Download LLM Package` creates an on-demand `.zip` of curated
  ...
  regulatory crosswalks, model-type writing guidance, citable-evidence and
  do-not-cite indexes, operator prompt variants, citation rules, tone profiles,
  interpretation briefs, a quality rubric, a redaction policy, a model facts
  digest, a human review checklist, generated-run code, run configuration,
  selected run evidence, DOCX build instructions, a model document style guide,
  a DOCX quality checklist, a figure placement manifest, section-specific
  evidence folders, a package build profile, and capped HTML plus
  document-ready PNG chart assets. If chart assets were already prepared through
  `Download Individual Images` in the same session, the LLM package reuses the
  eligible chart files instead of rendering them again. Raw input snapshots,
  row-level predictions, serialized model binaries, monitoring bundles, and
  full code snapshots are excluded by default.
- Step 5 `Download Individual Images` creates standalone chart PNG and HTML
  files only when requested. Normal workflow runs no longer write duplicate
  per-chart files, which keeps model execution focused on the report and core
  artifacts. The package uses the same report-size controls that govern the
...
- Individual chart `.html` and `.png` files are not written during normal GUI
  workflow runs. Use Step 5 `Download Individual Images` when separate chart
  files are needed. The generated HTML files share one Plotly JavaScript file,
  and PNG rendering uses Plotly's batch image writer when available.
...
Step 5 also offers on-demand LLM documentation and individual-chart downloads.
These packages are not written as part of every run; they are prepared when the
user clicks `Download LLM Package` or `Download Individual Images` in the Step 5
Export card. The LLM package is optimized for LLM drafting through evidence
maps, approved claims, documentation gaps, regulatory crosswalks, target
document schema, evidence-strength policy, document completion rules,
controlled vocabulary, draft-validation rules, two-pass prompts, citation
...
LLM writes the technical document. Prompt 2 directs the LLM to create
`model_methodology.docx` when the LLM environment supports Word file generation;
otherwise it falls back to Markdown with explicit figure placeholders.
