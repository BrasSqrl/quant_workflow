...
| `Download LLM Package` | completed run snapshot, exported artifact paths, and capped DOCX-ready chart assets | `llm_documentation_package.build_llm_documentation_package_from_payload(...)`, `figure_exports.build_figure_export_assets(...)`, `render_decision_summary(...)` | downloaded `.zip` organized into numbered folders with LLM-readable context, model facts digest, section evidence map, approved claims, documentation gaps, regulatory crosswalk, document schemas, evidence controls, citation boundaries, prompt variants, interpretation briefs, human review controls, TOC drop zone, DOCX workflow files, figure/table placement manifests, deterministic DOCX helper scripts, selected non-row-level run evidence, capped HTML chart assets, and a small document-ready PNG subset; full PNG chart export is available through `Download Individual Images` |
