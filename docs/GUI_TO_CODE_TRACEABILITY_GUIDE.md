...
| `Download LLM Package` | completed run snapshot, exported artifact paths, and capped DOCX-ready chart assets | `llm_documentation_package.build_llm_documentation_package_from_payload(...)`, `figure_exports.build_figure_export_assets(...)`, `render_decision_summary(...)` | downloaded `.zip` containing LLM-readable context, model facts digest, section evidence map, approved claims, documentation gaps, regulatory crosswalk, target document schema, evidence-strength policy, citable-evidence and do-not-cite indexes, completion rules, controlled vocabulary, draft-validation rules, quality rubric, redaction policy, validator script, operator prompt variants, citation rules, tone profiles, interpretation briefs, human review checklist, table-of-contents drop zone, DOCX build instructions, model document style guide, DOCX quality checklist, figure placement manifest, section-specific evidence folders, package build profile, selected non-row-level run evidence, capped HTML chart assets, and a small document-ready PNG subset; full PNG chart export is available through `Download Individual Images` |
...
- The LLM package is an on-demand Step 5 download. It does not rerun the model
  ...
  includes capped lightweight HTML chart assets, a small document-ready PNG
  subset, DOCX authoring controls, and reuses prepared individual chart assets
  when available in the same UI session.
