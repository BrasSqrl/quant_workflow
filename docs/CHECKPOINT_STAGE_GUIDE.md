...
| `export_package` | Export package | Required | Writes the model object, reports, tables, manifests, rerun code, checkpoints, predictions, regulatory outputs, validation checklist, traceability map, and report payload audit. Monitoring handoff bundles are no longer written automatically; create them from Step 5 with `Download OM Package` after a completed `fit_new_model` run. | Step 2 `Output Options`; Step 2 `Diagnostics & Exports`; export profile; input-driven tabular output format; individual figure export; enhanced report visuals; Advanced Visual Analytics; report-size controls. |
