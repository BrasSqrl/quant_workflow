...
| `export_package` | Export package | Required | Writes the model object, reports, tables, manifests, rerun code, checkpoints, predictions, regulatory outputs, validation checklist, traceability map, report payload audit, and monitoring handoff bundle when applicable. | Step 2 `Output Options`; Step 2 `Diagnostics & Exports`; export profile; input-driven tabular output format; individual figure export; enhanced report visuals; Advanced Visual Analytics; report-size controls. |
