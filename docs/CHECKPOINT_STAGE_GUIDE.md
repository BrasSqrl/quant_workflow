...
| `export_package` | Export package | Required | Writes the model object, reports, tables, manifests, rerun code, checkpoints, predictions, regulatory outputs, validation checklist, traceability map, and report payload audit. Monitoring handoff bundles and individual chart-file packages are no longer written automatically; create them from Step 5 with `Download OM Package` or `Download Individual Images` after a completed run. | Step 2 `Output Options`; Step 2 `Diagnostics & Exports`; export profile; input-driven tabular output format; enhanced report visuals; Advanced Visual Analytics; report-size controls. |
