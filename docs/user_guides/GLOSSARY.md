...
| LLM documentation package | Step 5 on-demand zip download with curated non-row-level evidence, section evidence maps, approved claims, documentation gaps, regulatory crosswalks, target document schema, evidence-strength policy, completion rules, controlled vocabulary, draft-validation rules, prompt variants, citation rules, interpretation briefs, quality rubric, table-of-contents drop zone, and review checklist for LLM-assisted model methodology drafting. |
