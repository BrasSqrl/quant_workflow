...
- Step 5 `Download LLM Package` creates an on-demand `.zip` of curated
  ...
  configuration, selected run evidence, and the same individual chart assets
  generated by `Download Individual Images`. Raw input snapshots, row-level
...
`THREE_PROMPTS_FOR_LLM_USE.txt`, a copy/paste plain-text prompt sequence for
planning, drafting, and validating the LLM output.
