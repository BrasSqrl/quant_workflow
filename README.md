...
- Step 5 `Download LLM Package` creates an on-demand `.zip` of curated
  ...
  selected run evidence, and the same individual chart assets generated by
  `Download Individual Images`. Raw input snapshots, row-level
...
lightweight draft validator script, human review checklists, and the same chart
PNG/HTML assets produced by `Download Individual Images`. It also includes
